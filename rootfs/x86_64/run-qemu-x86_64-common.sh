...
__runkernel_common()
{
    ...
    if [[ ${linux_version_code} -ge $(kernel_version 5 15) ]]; then
	if [[ ${runall} -ne 0 ]]; then
	    runkernel defconfig "${prefix}smp2:net=e1000:mem512:ata:fstest=ntfs" IvyBridge q35 rootfs.ext2
	    retcode=$((retcode + $?))
	    checkstate ${retcode}
	fi
    fi

    runkernel defconfig ${prefix}smp:net=e1000:mem256:ata:fstest=xfs Broadwell-noTSX q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}
    ...
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp8:net=ne2k_pci:mem1G:usb-hub SandyBridge q35 "rootfs.f2fs"
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp:tpm-tis:net=pcnet:mem2G:usb-uas Haswell q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig "${prefix}smp2:tpm-tis:net=rtl8139:efi:mem4G:sdhci-mmc" Skylake-Client q35 "rootfs.erofs"
    retcode=$((retcode + $?))
    checkstate ${retcode}
    ...
    runkernel defconfig ${prefix}smp4:net=tulip:efi32:mem256:scsi[DC395]:fstest=hfs Conroe q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp2:net=tulip:efi:mem256:scsi[DC395] Denverton q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp:tpm-crb:net=tulip:mem256:scsi[DC395] EPYC-Milan q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp:net=virtio-net:mem512:scsi[AM53C974] Nehalem q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}

    runkernel defconfig ${prefix}smp2:tpm-crb:net=usb-ohci:efi:mem1G:scsi[53C810] Westmere-IBRS q35 rootfs.iso
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp4:tpm-tis:net=e1000-82544gc:efi32:mem2G:scsi[53C895A] Skylake-Server q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp:pci-bridge:net=usb-uhci:mem4G:scsi[FUSION] EPYC pc rootfs.btrfs
    retcode=$((retcode + $?))
    checkstate ${retcode}
    ...
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp4:net=i82558b:efi:mem8G:virtio Broadwell q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}
    runkernel defconfig ${prefix}smp2:net=i82558a:efi32:mem1G:virtio Nehalem q35 rootfs.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}

    runkernel defconfig ${prefix}smp2:net=usb-ohci:efi:mem1G:scsi[53C810] Cooperlake q35 rootfs-x86.ext2
    retcode=$((retcode + $?))
    checkstate ${retcode}
